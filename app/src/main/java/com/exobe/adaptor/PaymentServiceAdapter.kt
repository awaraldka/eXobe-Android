...
class PaymentServiceAdapter (
...
): RecyclerView.Adapter<PaymentServiceAdapter.ViewHolder>(){
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PaymentServiceAdapter.ViewHolder {
        val view = LayoutInflater.from(context).inflate(R.layout.notification_modallayout, parent, false)
        return ViewHolder(view)
    }

    @RequiresApi(Build.VERSION_CODES.O)
    override fun onBindViewHolder(holder: PaymentServiceAdapter.ViewHolder, position: Int) {
        try {
            var userData = data[position]

            holder.txt1.text = "Hi ${userData.paidToDetails[0].firstName + " " + userData.paidToDetails[0].lastName} ,R ${userData.orderAmount.toString()}  received from " +
                    "${userData.userDetails.firstName+ " " +userData.userDetails.lastName} for Service Request ID ${userData.orderDetails.orderId}"
